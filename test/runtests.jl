...
sloc, le_loc, chord, tw_aero_d, af_shape_file, int_str_file, ib_sp_stn,
    ob_sp_stn, ib_ch_loc, ob_ch_loc = PreComp.readmain("test01_composite_blade.pci")

naf = length(sloc)
input = Array{PreComp.Input,1}(undef, naf)
output = Array{PreComp.Output,1}(undef, naf)
tw_prime_d = PreComp.tw_rate(naf, sloc, tw_aero_d)
for i = 1:naf
    xu, yu, xl, yl = PreComp.readprecompprofile(af_shape_file[i])
    e1, e2, g12, nu12, rho, name = PreComp.readmaterials("materials.inp")
    ...
    PreComp.readcompositesection(int_str_file[i], locW)

    input[i] = PreComp.Input(chord[i], tw_aero_d[i], tw_prime_d[i], le_loc[i],
    ...
    output[i] = PreComp.properties(input[i])
end
